...
public class Seal {
   ...
   * 绘制圆弧形文字
   */
  private static void drawArcFont4Circle(Graphics2D g2d, int circleRadius, SealFont font,
      boolean isTop) {
    if (font == null) {
      return;
    }
    ...
    for (int i = 0; i < textLen; i++) {
      ...
      g2d.setFont(f2);
      g2d.drawString(font.getText().substring(i, i + 1),
          (float) (circleRadius + thetaX + INIT_BEGIN),
   ...
   * 绘制椭圆弧形文字
   */
  private static void drawArcFont4Oval(Graphics2D g2d, SealCircle sealCircle, SealFont font,
      boolean isTop) {
    if (font == null) {
      return;
    }
    ...
    for (double i = startAngle + step; num < alCount; i += step) {
      ...
      double x = radiusX * Math.cos(angR) + radiusWidth, y =
          radiusY * Math.sin(angR) + radiusHeight;
  ...
  public boolean draw2(String pngPath) throws Exception {
    ...
    if (mainFont.getText().length() == 2) {
      if (stamp != null && stamp.trim().length() > 0) {
        ...
        marginW += Math.abs(rectangle.getCenterX()) * 2 + (mainFont.getSpace() == null ? INIT_BEGIN
            : mainFont.getSpace());
   ...
   * 画三字
   */
  private static BufferedImage drawThreeFont(BufferedImage bi, Graphics2D g2d, SealFont font,
      int lineSize, int imageSize, int fixH, int fixW, boolean isWithYin) {
    fixH -= 9;
    ...
    if (isWithYin) {
      ...
      marginW +=
          rectangle.getCenterX() * 2 + (font.getSpace() == null ? INIT_BEGIN : font.getSpace());
    } else {
      marginW +=
          rectangle.getCenterX() * 2 + (font.getSpace() == null ? INIT_BEGIN : font.getSpace());
   ...
   * 画四字
   */
  private static BufferedImage drawFourFont(BufferedImage bi, SealFont font, int lineSize,
      int imageSize, int fixH, int fixW) {
    int marginW = fixW + lineSize;
    //拉伸
    BufferedImage nbi = new BufferedImage(imageSize, imageSize, bi.getType());
    Graphics2D ng2d = nbi.createGraphics();
    ...
    g2d.drawString(font.getText().substring(2, 3), marginW, marginH);
    int oldW = marginW;
    marginW += Math.abs(rectangle.getCenterX()) * 2 + (font.getSpace() == null ? INIT_BEGIN
        : font.getSpace());
